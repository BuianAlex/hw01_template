...
const createLibsCss = gulp.series(createCss,function (cd) {
	gulp.src('src/css/libs.css', { allowEmpty: true }) // Выбираем файл для минификации
		.pipe(cssnano()) // Сжимаем
		.pipe(rename({ suffix: '.min' })) // Добавляем суффикс .min
		.pipe(gulp.dest('build/css')); // Выгружаем в папку src/css
	cd();
});
...
const buildJS = function (cd) {
	gulp.src(['./src/js/app.js'],{ allowEmpty: true } )
		.pipe(webpackStream({
			output: {
				filename: 'main.js',
			},
...
// Переносим шрифты 
const buildFonts = function (cd) {
	gulp.src('src/fonts/**/*') 
	.pipe(gulp.dest('build/fonts'))
	cd();
}		

const watch = gulp.series(buildHtml, createCss, libScripts, buildJS,  buildFonts, createBrowserSync, function (cd) {
